...
* Introduce the table with a phrase that describes its contents and ends in a colon.
For example, `the following default options are available:`
...
* Group rows by entering all content on the same line, separated by vertical bars (|), or by entering the content for a row in a block separated by whitespace.
For example: 
+
----
|====
...
|====
----
* If your table has more than 2 columns, you must set the proportional width of each column with the `[cols=""]` attribute before the opening `|====` for your table.
For example, if a table with 4 columns had `[cols="1,1,1,2"]`, the last column would be 2× the width of all the other columns. 

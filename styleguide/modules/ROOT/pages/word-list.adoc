...
| Capella Analytics | Proper product name for analytics on Capella. Write as written. Do not use Columnar. Refer to specific instances of Capella Analytics as <<cluster,clusters>>.
...
| [[cluster]]cluster | Preferred term, if referring to Couchbase Server or Couchbase Capella operational or Capella Analytics. We no longer use <<database,database>>.
...
| [[database]]database | Do not capitalize when referring to a database in Capella Analytics. Do not use to refer to Capella operational <<clusters,clusters>>.
